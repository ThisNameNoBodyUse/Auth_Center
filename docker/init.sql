...
CREATE TABLE IF NOT EXISTS applications (
    ...
    UNIQUE KEY uk_app_name_deleted (name, deleted_at),
...
CREATE TABLE IF NOT EXISTS users (
    ...
    updated_at DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP COMMENT '更新时间',
    deleted_at DATETIME NULL DEFAULT NULL COMMENT '软删除时间',
    UNIQUE KEY uk_user_app_username_deleted (app_id, username, deleted_at),
    UNIQUE KEY uk_user_app_email_deleted (app_id, email, deleted_at),
...
CREATE TABLE IF NOT EXISTS roles (
    ...
    updated_at DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP COMMENT '更新时间',
    deleted_at DATETIME NULL DEFAULT NULL COMMENT '软删除时间',
    UNIQUE KEY uk_role_app_code_deleted (app_id, code, deleted_at),
...
CREATE TABLE IF NOT EXISTS permissions (
    ...
    resource VARCHAR(64) NULL COMMENT '资源类型，保留兼容（当前固定为api）',
    action VARCHAR(64) NULL COMMENT '操作类型（当前用于HTTP方法）',
    description VARCHAR(255) NULL COMMENT '描述',
    status TINYINT NOT NULL DEFAULT 1 COMMENT '状态 1启用 0禁用',
    created_at DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP COMMENT '创建时间',
    updated_at DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP COMMENT '更新时间',
    deleted_at DATETIME NULL DEFAULT NULL COMMENT '软删除时间',
    UNIQUE KEY uk_perm_app_code_deleted (app_id, code, deleted_at),
...
CREATE TABLE IF NOT EXISTS apis (
    ...
    updated_at DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP COMMENT '更新时间',
    deleted_at DATETIME NULL DEFAULT NULL COMMENT '软删除时间',
    UNIQUE KEY uk_api_app_path_method_deleted (app_id, path, method, deleted_at),
...
INSERT INTO permissions (app_id, name, code, resource, action, description, status) VALUES
                                                                                        ('default-app', '用户管理', 'user:manage', 'api', 'GET', '用户管理权限', 1),
                                                                                        ('system-admin', '应用管理', 'app_manage', 'api', 'GET', '应用的所有操作权限', 1)
    ON DUPLICATE KEY UPDATE name=VALUES(name);

-- 插入默认API
INSERT INTO apis (app_id, path, method, description, permission_id) VALUES
                                                                        ('system-admin', '/api/v1/apps', 'POST', '创建应用', (SELECT id FROM permissions WHERE code='app_manage' AND app_id='system-admin')),
                                                                        ('system-admin', '/api/v1/apps/*', 'PUT', '更新应用', (SELECT id FROM permissions WHERE code='app_manage' AND app_id='system-admin')),
                                                                        ('system-admin', '/api/v1/apps/*', 'DELETE', '删除应用', (SELECT id FROM permissions WHERE code='app_manage' AND app_id='system-admin'))
    ON DUPLICATE KEY UPDATE description=VALUES(description);

-- 为默认应用管理员角色分配用户管理权限（避免使用硬编码ID）
INSERT INTO role_permissions (role_id, permission_id, app_id)
SELECT r.id, p.id, 'default-app'
FROM roles r
         JOIN permissions p ON p.app_id = 'default-app'
WHERE r.app_id = 'default-app' AND r.code = 'admin' AND p.code = 'user:manage'
...
FROM roles r
         JOIN permissions p ON p.app_id = 'system-admin'
WHERE r.app_id = 'system-admin' AND r.code = 'super_admin' AND p.code = 'app_manage'
    ON DUPLICATE KEY UPDATE permission_id = VALUES(permission_id);
